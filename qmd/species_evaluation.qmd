...
[`r str_to_upper("Pulled from GBIF and then manually reviewed – e.g., USDA Plants, scientific literature, etc")`]
...
```{r}    
#| label: cont-range
#| fig-cap: "Species range within the continental United States"
...
```{r}
get_hab_types <- tar_read(crosswalk_habitats_to_species) |>
  filter(taxon_id == t_id) |>
...
if (nrow(get_hab_types) > 0) {
  ...
  hab_type_narrative <- str_to_upper("There are no habitat types available for this species from Naturserve that crosswalk to MPSG defined ecosystems. ")
}
```

`r hab_type_narrative`

[`r str_to_upper("Checklist of categories from Ecology Group pulled in from spreadsheet – this will get pulled in automatically from Mike’s NatureServe pull + crosswalk, but will still require manual review for accuracy (especially for the plantation category). See appendix for landscape-scale habitat availability and distribution in the plan area.")`]
...
#### What are the trends, both long-term and recent, in the distribution, abundance, and quality of the fine-scale habitat features?

[DEFAULT: There is limited information about the distribution, abundance and quality of this species’ microhabitat in the plan area.]
...
#### What are the on-going threats that may shape the future distribution, abundance, and quality of the fine-scale habitat features?  

[`r str_to_upper("Provide narrative discussing species-specific information")`]

### Population 

#### Abundance and distribution of species in the plan area

[DEFAULT: There are no known population estimates for the species in the plan area.]

[`r str_to_upper("Pull in information from species records, literature, reports, species distribution models, etc.")`]
