...
t_path <- file.path("T:/FS/NFS/PSO/MPSG/2024_NebraskaNFG", 
                    "1_PreAssessment/Projects/SpeciesList_NNFG")
# Geodatabase
pre_gdb <- file.path("T:/FS/NFS/PSO/MPSG/2024_NebraskaNFG", 
                     "1_PreAssessment/Data/NNFG_PreAssessmentData.gdb")


#-- Packages
# R packages used in this script
pkgs <- c("arcgisbinding",  # Interface with ArcGIS
...
#-- Spatial Data
gdb_path <- file.path("T:/FS/NFS/PSO/MPSG/2024_CimarronComancheNG", 
...
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "SDNHP_all"), 
                         data = sf_cnhp, 
                         overwrite = TRUE)
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "CONHP_FS"), 
                         data = sf_cnhp_unit, 
                         overwrite = TRUE)
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "KSNHI_all"), 
                         data = sf_ksnhp, 
                         overwrite = TRUE)
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "KSNHI_FS"), 
                         data = sf_ksnhp_unit, 
                         overwrite = TRUE)
...
gbif_short <- c("occurrenceID", "taxonID", "scientificName", 
...
#-- Write Data
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', 
                                          "OverviewSpp_GBIF_all"), 
                         data = dplyr::filter(sf_gbif, 
...
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', 
                                          "OverviewSpp_GBIF_FS"), 
                         data = dplyr::filter(sf_gbif_unit, 
                                              taxonKey %in% ovrvw_gbif_ids) |> 
                           dplyr::select(dplyr::all_of(gbif_short)), 
                         overwrite = TRUE)
...
idb_short <- c("uuid", "taxonid", "canonicalname", "commonname", 
...
# SEINet ----
load(file.path(t_path, 'reproduce', "seinet.RData"))

#-- Prep data
ovrvw_sei_ids <- id_vec(spp_ovrvws$SEI_taxonID)
buff_sei_ids <- id_vec(buff_list$SEI_taxonID)
sei_short <- c("occurrenceID", "taxonID", "scientificName", 
...
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', 
                         ...
                         data = dplyr::filter(sf_seinet_unit, 
                                              taxonID %in% ovrvw_sei_ids) |> 
                           dplyr::select(dplyr::all_of(sei_short)), 
                         overwrite = TRUE)
...
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "IMBCR_all"), 
                         data = sf_imbcr, 
                         overwrite = TRUE)

arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "IMBCR_FS"), 
                         data = sf_imbcr_unit, 
                         overwrite = TRUE)                         
...
arcgisbinding::arc.write(path = file.path(pre_gdb, 'species', "CPW_bats"), 
                         data = sf_cpwbats, 
                         overwrite = TRUE)
