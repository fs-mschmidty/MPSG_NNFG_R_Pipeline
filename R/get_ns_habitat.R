#' This function makes a call to the NatureServe API using the NatureServe State List and returns all habitats associated with any given species.
...
get_ns_habitat <- function(ns_state_list, el_list) {
  ...
  ns_el_data <- ns_state_list |>
    ...
    mutate(ns_taxon_api_url = glue::glue("https://explorer.natureserve.org/api/data/taxon/{api_shortcode}")) |>
    distinct() |>
    group_by(taxon_id) |>
    mutate(n = n()) |>
    ungroup() |>
  ...
  natureserv_get_hab_data <- function(x) {
    ...
    if (resp$status_code == 200) {
      ...
      get_all_habs <- function(x, ls_json) {
        hab_type <- x
        hab_list <- ls_json[glue::glue("species{str_to_title(hab_type)}Habitats")]
