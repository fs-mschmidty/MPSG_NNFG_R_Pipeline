build_transient_birds <- function(x, nnfg_aoa) {
  ...
  transient_analysis <- lapply(eligible_birds_w_dist, get_transients, aoa) |>
    bind_rows()

  transient_analysis <- eligible_birds_w_dist |>
    left_join(transient_analysis, by = "scientific_name")

  final_bird_data <- list()

  final_bird_data[["transient_analysis"]] <- transient_analysis
  final_bird_data[["birds_wo_distribution_models"]] <- eligible_birds_wo_dist
